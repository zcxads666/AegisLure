...
if [[ -n "${HP_PROFILES:-}" ]]; then set_env_value HP_PROFILES "$HP_PROFILES"; elif ! grep -q '^HP_PROFILES=' .env; then set_env_value HP_PROFILES 'new-api,vllm,ollama,sglang,sub2api'; fi
