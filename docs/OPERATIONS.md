...
`hpctl import` is an offline, bounded JSONL importer for the allowlisted local products (`new-api`, `vllm`, `ollama`, `sglang`, `localai`, and `sub2api`). It accepts only the documented event fields, requires an IP address, redacts body previews, stores an explicitly supplied bounded raw-request object for owner/admin audit, and records source product/schema/file/offset/hash provenance. Re-running the same file identity and byte offsets is idempotent. It never downloads models, opens a source URL, or forwards imported content to an external service.
