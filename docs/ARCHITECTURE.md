...
When IPinfo API is configured, the authenticated control-plane
...
1. Before public deployment, validate the host firewall/VPN restriction for the random admin port, the no-egress behavior of deployments without an IPinfo key, the explicitly allowed IPinfo API egress when a key is configured, and the broker's fixed OAuth egress separately from unit tests.
