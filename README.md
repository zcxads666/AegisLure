...
AegisLure 提供五类安全的 clean-room 协议仿真：New API、vLLM、Ollama、SGLang 和 LocalAI。它记录产品识别、模型目录浏览、认证结果、合成调用、响应消费和虚拟效果验证，但不会运行模型、执行 prompt 工具、解析攻击者上传的模型/媒体、访问 URL 或连接真实模型供应商；默认使用本地 MaxMind GeoLite2 City + ASN 数据库查询公网 IP，也可在管理设置中切换到 IPinfo Lite。
...
- 本地 MaxMind GeoLite2 City + ASN 公网 IP 查询：数据库只读复用并缓存结果，缺失、未命中或查询失败时回退到本地地址分类和“未知”；管理设置可切换到 IPinfo Lite，key 只保存在后端运行配置；
