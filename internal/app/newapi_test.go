...
func TestNewAPIOAuthSimulationIsConfigurableAndRecordsNoProviderCredentials(t *testing.T) {
	...
	if resp.StatusCode != http.StatusNotFound {
		t.Fatalf("disabled registration OAuth simulation status = %d", resp.StatusCode)
	}
	...
	resp, statusBody = doRawJSON(t, public, http.MethodGet, "/api/status", nil, nil)
	if resp.StatusCode != http.StatusOK || !strings.Contains(string(statusBody), `"oauth_register_enabled":true`) || !strings.Contains(string(statusBody), `"oauth_login_enabled":true`) || !strings.Contains(string(statusBody), `"github_oauth":true`) {
	...
	resp, simulation = doJSON(t, public, http.MethodPost, "/api/oauth/state", map[string]string{"provider": "github", "intent": "register"})
	if resp.StatusCode != http.StatusUnauthorized || simulation["success"] != false || simulation["code"] != "OAUTH_UNAVAILABLE" {
		t.Fatalf("registration OAuth simulation response = %d %#v", resp.StatusCode, simulation)
	}
	...
	oauthSurfaces := make(map[string]bool)
	for _, event := range events {
		...
		surface := event.Metadata["oauth_surface"]
		oauthSurfaces[surface] = true
		if (surface != "login" && surface != "register") || event.Metadata["oauth_outcome"] != "rejected" || event.Metadata["risk_equivalent"] != "newapi.user.login.failed" || !containsString(event.ReasonCodes, "newapi_login_failed") || strings.Contains(strings.ToLower(event.BodyPreview), "access_token") || strings.Contains(strings.ToLower(event.BodyPreview), "client_secret") {
			t.Fatalf("OAuth event was not bounded and rejected safely: %+v", event)
		}
	}
	if loginEvent == nil || oauthEvent == nil || !oauthSurfaces["login"] || !oauthSurfaces["register"] {
		t.Fatalf("authentication risk events were not recorded for both surfaces: login=%+v oauth=%+v surfaces=%v", loginEvent, oauthEvent, oauthSurfaces)
	}
