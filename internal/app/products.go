...
func publicUsageEvents(events []model.Event, sessionID, userID string) []map[string]any {
	...
	for _, event := range usageEvents {
		...
		result = append(result, map[string]any{
			...
			"group_id":                 int64(1),
			"group_name":               "default",
			"model_provider":           event.Metadata["model_provider"],
			"request_content_kind":     event.Metadata["request_content_kind"],
			"request_content_preview":  event.Metadata["request_content_preview"],
			"response_content_preview": event.Metadata["response_content_preview"],
		})
	}
	return result
}
