...
func defaultDetectorRulePack() packs.DetectorRulePack {
	...
	sub2APIGatewayRoutes := []string{"sub2api.gateway.messages", "sub2api.gateway.count_tokens", "sub2api.gateway.chat", "sub2api.gateway.responses", "sub2api.gateway.embeddings", "sub2api.gateway.completions", "sub2api.gateway.billing", "sub2api.gateway.alpha_search", "sub2api.gateway.usage", "sub2api.gateway.live"}
	...
	return packs.DetectorRulePack{
		...
		Rules: []packs.DetectorRule{
			...
			{ID: "SUB2API_AUTH_SSRF_V1", Type: "atomic", ReasonCode: "sub2api_auth_ssrf_probe", Score: 55, Confidence: "high", URLClasses: []string{"loopback", "unspecified", "link_local", "private", "file_scheme"}, Where: detectorAll(detectorProduct(model.ProductSub2API), detectorRoute(sub2APIGatewayRoutes...))},
			{ID: "SUB2API_OAUTH_ENUMERATION_V1", Type: "atomic", ReasonCode: "sub2api_oauth_provider_enumeration", Score: 40, Confidence: "medium", Where: detectorAll(detectorProduct(model.ProductSub2API), detectorRoute("sub2api.auth.oauth"), detectorAny(detectorAuth("missing"), detectorAuth("invalid")))},
			{ID: "SUB2API_GATEWAY_UNAUTHORIZED_ENUM_V1", Type: "atomic", ReasonCode: "sub2api_gateway_unauthorized_enumeration", Score: 35, Confidence: "medium", Where: detectorAll(detectorProduct(model.ProductSub2API), detectorRoute(sub2APIGatewayRoutes...), detectorAny(detectorAuth("missing"), detectorAuth("invalid")))},
			{ID: "SUB2API_GATEWAY_KEY_REUSE_V1", Type: "atomic", ReasonCode: "sub2api_gateway_honey_key_reuse", Score: 55, Confidence: "high", Where: detectorAll(detectorProduct(model.ProductSub2API), detectorRoute(sub2APIGatewayRoutes...), detectorAuth("leaked_key_reused"))},
			{ID: "SUB2API_QUOTA_PROBE_V1", Type: "atomic", ReasonCode: "sub2api_quota_probe", Score: 55, Confidence: "high", Where: detectorAll(detectorProduct(model.ProductSub2API), detectorRoute(sub2APIGatewayRoutes...), detectorBody(`(?i)(?:max_tokens|max_output_tokens|n|best_of).{0,64}(?:[1-9][0-9]{6,}|1e\+?0?6)`))},
			{ID: "SUB2API_ACCOUNT_TO_GATEWAY_V1", Type: "sequence", ReasonCode: "sub2api_account_to_gateway_chain", Score: 35, Confidence: "medium", Within: "30m", SequenceMode: "unordered", Steps: []string{"sub2api.user.register.success", "sub2api.user.login.success", "sub2api.key.created", "sub2api.models.listed", "sub2api.gateway"}},
