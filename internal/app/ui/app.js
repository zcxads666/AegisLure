...
const LIST_DEFAULTS = {
  ...
  invocations: { page: 1, page_size: 10, q: '', level: '', auth: '', execution: '' },
  chains: { page: 1, page_size: 10, q: '' },
  indicators: { page: 1, page_size: 10, q: '', risk_level: '', sort: 'latest' },
}
...
function App() {
  ...
  const loadRoute = useCallback(async (target = route, quiet = false, overrideParams = null) => {
    ...
    try {
      ...
      else if (target === 'indicators') { const params = overrideParams || listParamsRef.current.indicators; const query = new URLSearchParams({ page: String(params.page || 1), page_size: String(params.page_size || 10) }); if (params.q) query.set('q', params.q); if (params.risk_level) query.set('risk_level', params.risk_level); if (params.sort) query.set('sort', params.sort); if (params.min_score !== '' && params.min_score != null) query.set('min_score', String(params.min_score)); loaded = await request(`indicators?${query}`, options); if (!current()) return null; setData((value) => ({ ...value, indicators: loaded.items || [], pagination: { ...value.pagination, indicators: responsePagination(loaded) } })) }
