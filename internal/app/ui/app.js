...
function ServerObservationsPage({ events = [], pagination, onRefresh, onOpenEvent, onSearch, onPageChange, onDelete }) {
  ...
  const columns = [
    ...
    { label: '原始路由', render: (row) => html`<span class="route-cell">${rawRoute(row)}</span>` },
...
function App() {
  ...
  const deleteListItem = useCallback(async (target, identifier) => {
    ...
    const endpoints = { observations: 'events', invocations: 'invocations', chains: 'interaction-chains', indicators: 'indicators' }
    ...
    try {
      ...
      if (params.page > 1 && result && Array.isArray(result[target === 'observations' ? 'events' : target === 'invocations' ? 'invocations' : target === 'chains' ? 'chains' : 'items']) && result[target === 'observations' ? 'events' : target === 'invocations' ? 'invocations' : target === 'chains' ? 'chains' : 'items'].length === 0 && Number(result.total || 0) > 0) await loadList(target, { page: params.page - 1 })
