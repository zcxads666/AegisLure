...
function ChartHoverDetail({ item, kind = 'generic', empty = '移动到图表项目上查看详细数据。' }) {
  if (!item) return html`<div class="chart-hover-detail is-muted" aria-live="polite">${empty}</div>`
  if (kind === 'activity') return html`<div class="chart-hover-detail" aria-live="polite"><strong>${item.label || '时间段'}</strong><span>总事件 ${formatNumber(item.count)} · 风险触发 ${formatNumber(item.risk_count)} · 峰值 ${formatNumber(item.peak_score)} 分</span></div>`
  if (kind === 'country') return html`<div class="chart-hover-detail" aria-live="polite"><strong>${item.name || '未知'}</strong><span>${item.country_code || '无国家码'} · ${item.continent || '区域待定'} · ${formatNumber(item.count)} 个风险指标 · ${item.percentage || 0}% · ${geoSourceLabel(item.geo_source)}</span></div>`
  if (kind === 'honeypot') return html`<div class="chart-hover-detail" aria-live="polite"><strong>${profileLabel(item.name)}</strong><span>事件 ${formatNumber(item.count)} 次 · ${item.percentage || 0}% · 风险 ${formatNumber(item.risk_count)} 次 · 类型内风险率 ${item.risk_percent || 0}%</span></div>`
  return html`<div class="chart-hover-detail" aria-live="polite"><strong>${item.name || item.key || '数据项'}</strong><span>${formatNumber(item.count)} 次 · ${item.percentage || 0}%</span></div>`
}

function ActivityChart({ items = [] }) {
  const [hovered, setHovered] = useState(null)
  const max = Math.max(1, ...items.map((item) => Number(item.count || 0)))
  return html`<div><div class="activity-chart" aria-label="近 24 小时事件量">${items.map((item, index) => html`<div class="activity-bar-wrap" key=${item.label || index} onMouseEnter=${() => setHovered(item)} onMouseLeave=${() => setHovered(null)}><div class="activity-value">${item.count || ''}</div><div class="activity-bar" style=${{ height: `${Math.max(5, (Number(item.count || 0) / max) * 100)}%` }}></div><small>${item.label}</small></div>`)}</div><${ChartHoverDetail} item=${hovered} kind="activity" empty="移动到柱状条查看该时间段的事件量。" /></div>`
}
...
function RiskDonut({ distribution = {} }) {
  ...
  return html`<${DistributionDonut} items=${items} centerLabel="IP 指标" />`
}
...
function DistributionDonut({ items = [], centerLabel = '事件总量' }) {
  ...
  return html`<div class="risk-donut-wrap"><div class="risk-donut donut-interactive" aria-label=${`${centerLabel}分布图`}><svg class="donut-svg" viewBox="0 0 120 120" role="img" aria-hidden="true">${total ? segments.map((item, index) => html`<path key=${item.key || index} class=${cn('donut-segment', active?.key === item.key && 'is-hovered')} d=${donutArcPath(item.start, item.end)} fill=${item.color} onMouseEnter=${() => setHovered(item)} onMouseLeave=${() => setHovered(null)}><title>${item.name || item.key} · ${formatNumber(item.count)} · ${item.percentage}%</title></path>`) : html`<circle class="donut-track" cx="60" cy="60" r="38"></circle>`}</svg><div class="donut-center"><div><strong>${formatNumber(active ? active.count : total)}</strong><small>${active ? active.name || active.key : centerLabel}</small></div></div></div><div class="donut-legend-stack"><div class="legend-list">${normalized.map((item, index) => { const percentage = item.percentage || (total ? Math.round((item.count / total) * 100) : 0); return html`<div key=${item.key || index} class=${cn(active?.key === item.key && 'is-hovered')} onMouseEnter=${() => setHovered(item)} onMouseLeave=${() => setHovered(null)}><i class="legend-dot" style=${{ background: colors[item.key] || palette[index % palette.length] }}></i><span>${item.name || item.key}</span><b>${formatNumber(item.count)} <small>${percentage}%</small></b></div>` })}</div><${ChartHoverDetail} item=${active} empty="移动到环形分段或图例查看占比明细。" /></div></div>`
}
...
function CountryDistribution({ items = [] }) {
  ...
  return html`<div class="country-list">${visible.map((item) => html`<div class=${cn('country-row', hovered?.key === item.key && 'is-hovered')} key=${item.key || item.name} onMouseEnter=${() => setHovered(item)} onMouseLeave=${() => setHovered(null)}><div class="country-row-head"><span><i class="country-bullet"></i>${item.name || '未知'}</span><b>${formatNumber(item.count)} <small>${item.percentage || 0}%</small></b></div><div class="share-progress"><i style=${{ width: `${Math.max(3, (Number(item.count || 0) / max) * 100)}%` }}></i></div></div>`)}</div><${ChartHoverDetail} item=${hovered} kind="country" empty="移动到来源区域条目查看国家码、洲和查询来源。" /><div class="panel-footnote">MaxMind 本地数据库不可用或查询失败时回退到本地/保留、文档地址或“未知”；切换到 IPinfo Lite 后，公网查询成功会显示 API 结果。</div>`
}
...
function HoneypotDistribution({ items = [] }) {
  ...
  return html`<div class="honeypot-list">${visible.map((item, index) => html`<div class=${cn('honeypot-row', hovered?.key === item.key && 'is-hovered')} key=${item.key || item.name} onMouseEnter=${() => setHovered(item)} onMouseLeave=${() => setHovered(null)}><div class="honeypot-row-head"><span class="honeypot-rank">${String(index + 1).padStart(2, '0')}</span><div><b>${profileLabel(item.name)}</b><small>${formatNumber(item.count)} 次 · 风险 ${formatNumber(item.risk_count)} 次</small></div><strong>${item.percentage || 0}%</strong></div><div class="share-progress"><i style=${{ width: `${Math.max(3, (Number(item.count || 0) / max) * 100)}%` }}></i></div></div>`)}</div><${ChartHoverDetail} item=${hovered} kind="honeypot" empty="移动到蜜罐类型条目查看事件与风险占比。" /><div class="panel-footnote">占比按事件次数计算；“风险”表示该类型中风险分 ≥ 30 的事件。</div>`
}
...
function SettingsPage({ username, ipinfo, onRotateEntry, onSaveIPInfo }) {
  ...
  const isIPInfo = provider === 'ipinfo_lite'
  ...
  const save = async (nextProvider = provider, nextToken = token, includeToken = false) => {
    ...
    try {
      ...
      setToken('')
      setMessage(nextProvider === 'maxmind' ? (maxmind.ready ? 'MaxMind GeoLite2 已启用。' : '已切换到 MaxMind；请先放置 City 与 ASN 数据库。') : (includeToken ? 'IPinfo Lite key 已保存；完整 key 不会回显。' : ipinfoConfigured ? 'IPinfo Lite 已保持启用。' : '已选择 IPinfo Lite；请输入 key 后启用。'))
    } catch (error) {
      setMessage(error.message || '地理情报设置保存失败。'); setMessageTone('error')
    } finally { setBusy(false) }
  }
  const providerOptions = (ipinfo?.available_providers || [{ id: 'maxmind', label: 'MaxMind GeoLite2 City + ASN' }, { id: 'ipinfo_lite', label: 'IPinfo Lite API' }]).map((item) => ({ value: item.id, label: item.label }))
  const statusTone = configured ? 'success' : 'warning'
  return html`<div class="page-stack"><${PageHeader} eyebrow="Workspace settings" title="管理设置" description="管理离线恢复凭据、管理端入口和 IP 地理情报来源。" /><div class="settings-grid"><${Panel} eyebrow="Account security" title="离线恢复" className="settings-form-panel"><div class="security-card"><div class="security-card-icon">${icon('key', 22)}</div><h3>${username || 'owner'} · 恢复凭据</h3><p>管理端不提供在线改密。请使用一次性 recovery code，或在服务器本机通过 hpctl 生成 rescue code 后完成恢复。</p><span class="instance-boundary"><i class="green-dot"></i>恢复码成功使用后立即失效</span></div><//><${Panel} eyebrow="Admin endpoint" title="管理入口"><div class="security-card"><div class="security-card-icon">${icon('key', 22)}</div><h3>随机隐藏路径</h3><p>当前入口路径不会在 API 响应中返回。轮换后会使现有管理会话失效，页面会跳转到新的入口。</p><button class="button button-danger-soft button-full" type="button" onClick=${rotate}>${icon('refresh', 16)}轮换管理入口</button></div><//><${Panel} eyebrow="IP intelligence" title="IP 地理情报" className="settings-form-panel"><div class="security-card ipinfo-settings-card"><div class="settings-card-heading"><div class="security-card-icon">${icon('globe', 22)}</div><${Badge} tone=${statusTone} dot=${true}>${configured ? '已就绪' : '未就绪'}<//></div><h3>选择公网 IP 查询来源</h3><p>本地、保留和文档地址始终直接离线识别；公网地址优先使用当前 provider，数据库/API 不可用时回退为“未知”。</p><${Select} label="查询 provider" value=${provider} onChange=${setProvider} options=${providerOptions} /><div class="button-group geoip-save-row"><${Button} variant="primary" size="sm" onClick=${() => save(provider, token, isIPInfo && Boolean(token.trim()))} disabled=${busy || (isIPInfo && !token.trim() && !ipinfoConfigured)}>${busy ? '保存中…' : '保存查询设置'}<//>${isIPInfo && ipinfoConfigured ? html`<${Button} variant="ghost" size="sm" onClick=${() => save('ipinfo_lite', '', true)} disabled=${busy}>清除并停用 key<//>` : null}</div>${isIPInfo ? html`<div class="geoip-provider-panel"><${TextInput} label="IPinfo Lite Token / Key" type="password" value=${token} onInput=${setToken} placeholder=${ipinfoConfigured ? `当前已配置（${ipinfo?.masked_token || '已隐藏'}），输入新 key 可替换` : '粘贴 IPinfo Lite token（不会返回原文）'} autoComplete="new-password" maxLength=${256} /><div class="ipinfo-facts"><span>接口</span><code>${ipinfo?.endpoint || 'https://api.ipinfo.io/lite/{ip}'}</code><span>超时 / 成功缓存</span><b>${ipinfo?.timeout_seconds || 2}s / ${Math.round((ipinfo?.cache_ttl_seconds || 86400) / 3600)}h</b><span>失败缓存 / 单次首页上限</span><b>${Math.round((ipinfo?.failure_cache_ttl_seconds || 300) / 60)}min / ${ipinfo?.dashboard_lookup_limit || 128} 个 IP</b></div><p class="panel-footnote">key 只提交到后端，浏览器不会读取或回显完整 token；查询数据来自 <a href="https://ipinfo.io/lite" target="_blank" rel="noreferrer">IPinfo Lite</a>。</p></div>` : html`<div class="geoip-provider-panel"><div class="geoip-status-grid"><div><span>City 数据库</span><b class=${maxmind.city_available ? 'is-ready' : 'is-missing'}>${maxmind.city_available ? '可用' : '未找到'}</b><small>${maxmind.city_file || 'GeoLite2-City.mmdb'}</small></div><div><span>ASN 数据库</span><b class=${maxmind.asn_available ? 'is-ready' : 'is-missing'}>${maxmind.asn_available ? '可用' : '未找到'}</b><small>${maxmind.asn_file || 'GeoLite2-ASN.mmdb'}</small></div></div><p class="panel-footnote">将官方 GeoLite2 City 与 ASN 文件放入数据目录的 <code>geoip</code> 子目录；更新文件后重启服务。</p><p class="panel-footnote">数据获取与许可说明见 <a href="https://dev.maxmind.com/geoip/geolite2-free-geolocation-data/" target="_blank" rel="noreferrer">MaxMind GeoLite2</a>。</p></div>`}${message ? html`<div class=${cn('form-message', messageTone === 'error' && 'error', messageTone !== 'error' && 'success')}>${message}</div>` : null}</div><//><${Panel} eyebrow="Data boundary" title="运行边界" className="settings-note-panel"><div class="boundary-list"><div>${icon('shield', 17)}<span><b>事件流受保留策略限制</b><small>SQLite 是权威记录，JSONL 镜像会按保留天数与条数清理。</small></span></div><div>${icon('lock', 17)}<span><b>敏感字段脱敏</b><small>凭据、密码和 token 只保留 keyed fingerprint。</small></span></div><div>${icon('spark', 17)}<span><b>合成响应</b><small>不会运行模型、访问 URL 或连接供应商。</small></span></div></div><//></div></div>`
}
...
function App() {
  ...
  const saveIPInfo = useCallback(async (provider, token, includeToken = false) => {
    ...
    showToast(provider === 'maxmind' ? 'MaxMind GeoLite2 查询已启用' : (includeToken && token ? 'IPinfo Lite 配置已保存' : 'IPinfo Lite 设置已更新'))
  ...
  const logout = async () => { try { await request('auth/logout', { method: 'POST' }) } catch (_) {} setAuth('login'); setUsername(''); setSelectedActor(null); setSelectedEvent(null); setData({ dashboard: null, instances: [], events: [], invocations: [], chains: [], indicators: [], packs: null, policies: null, ipinfo: null }); navigateTo('login', true) }
