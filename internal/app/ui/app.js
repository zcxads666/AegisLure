...
function App() {
  ...
  const instanceAction = async (instance, action, payload = {}) => {
    ...
    if (action === 'save-model-name') {
      setBusy(true); try { const result = await request(`instances/${instance.product}`, { method: 'PATCH', body: JSON.stringify(payload) }); const updated = result.instance; setData((current) => ({ ...current, instances: current.instances.map((item) => item.product === instance.product ? { ...item, ...updated, version: updated.display_version || item.version } : item) })); showToast(`${profileLabel(instance.product)} 模型名称已更新`) } catch (error) { showToast(error.message || '模型名称保存失败', 'error') } finally { setBusy(false) }; return
    }
