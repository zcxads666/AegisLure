...
function ChartHoverDetail({ item, kind = 'generic', empty = '移动到图表项目上查看详细数据。' }) {
  ...
  if (kind === 'country') return html`<div class="chart-hover-detail" aria-live="polite"><strong>${item.name || '未知'}</strong><span>${item.country_code || '无国家码'} · ${item.continent || '区域待定'} · ${formatNumber(item.count)} 个风险指标 · ${item.percentage || 0}% · ${item.geo_source === 'ipinfo_lite' ? 'IPinfo Lite' : '离线/回退'}</span></div>`
...
function DashboardPage({ dashboard, instances, onNavigate, onRefresh, onOpenEvent }) {
  ...
  const geoProvider = (dashboard.source_countries || []).some((item) => item.geo_source === 'ipinfo_lite') ? 'IPinfo Lite' : '离线/回退'
...
function SettingsPage({ username, ipinfo, onRotateEntry, onSaveIPInfo }) {
  ...
  const save = async (nextToken) => {
    setBusy(true); setMessage(''); setMessageTone('success')
    try {
      await onSaveIPInfo(nextToken)
      setToken('')
      setMessage(nextToken ? 'IPinfo Lite key 已保存；完整 key 不会回显。' : 'IPinfo Lite 查询已关闭，将使用离线分类回退。')
    } catch (error) {
      setMessage(error.message || 'IPinfo Lite 设置保存失败。'); setMessageTone('error')
    } finally { setBusy(false) }
  }
  return html`<div class="page-stack"><${PageHeader} eyebrow="Workspace settings" title="管理设置" description="管理离线恢复凭据、管理端入口和可选的 IP 地理情报查询。" /><div class="settings-grid"><${Panel} eyebrow="Account security" title="离线恢复" className="settings-form-panel"><div class="security-card"><div class="security-card-icon">${icon('key', 22)}</div><h3>${username || 'owner'} · 恢复凭据</h3><p>管理端不提供在线改密。请使用一次性 recovery code，或在服务器本机通过 hpctl 生成 rescue code 后完成恢复。</p><span class="instance-boundary"><i class="green-dot"></i>恢复码成功使用后立即失效</span></div><//><${Panel} eyebrow="Admin endpoint" title="管理入口"><div class="security-card"><div class="security-card-icon">${icon('key', 22)}</div><h3>随机隐藏路径</h3><p>当前入口路径不会在 API 响应中返回。轮换后会使现有管理会话失效，页面会跳转到新的入口。</p><button class="button button-danger-soft button-full" type="button" onClick=${rotate}>${icon('refresh', 16)}轮换管理入口</button></div><//><${Panel} eyebrow="IP intelligence" title="IPinfo Lite 地理情报" className="settings-form-panel"><div class="security-card ipinfo-settings-card"><div class="settings-card-heading"><div class="security-card-icon">${icon('globe', 22)}</div><${Badge} tone=${configured ? 'success' : 'neutral'} dot=${true}>${configured ? '已配置' : '未配置'}<//></div><h3>可选公网 IP 查询</h3><p>只对公网 IP 进行后端查询；本地、保留和文档地址直接离线识别。无 key、超时或查询失败时自动回退为“未知”。</p><${TextInput} label="IPinfo Lite Token / Key" type="password" value=${token} onInput=${setToken} placeholder=${configured ? `当前已配置（${ipinfo?.masked_token || '已隐藏'}），输入新 key 可替换` : '粘贴 IPinfo Lite token（不会返回原文）'} autoComplete="new-password" maxLength=${256} /><div class="button-group"><${Button} variant="primary" size="sm" onClick=${() => save(token)} disabled=${busy || !token.trim()}>${busy ? '保存中…' : '保存 key'}<//>${configured ? html`<${Button} variant="ghost" size="sm" onClick=${() => save('')} disabled=${busy}>清除并关闭<//>` : null}</div>${message ? html`<div class=${cn('form-message', messageTone === 'error' && 'error', messageTone !== 'error' && 'success')}>${message}</div>` : null}<div class="ipinfo-facts"><span>接口</span><code>${ipinfo?.endpoint || 'https://api.ipinfo.io/lite/{ip}'}</code><span>超时 / 成功缓存</span><b>${ipinfo?.timeout_seconds || 2}s / ${Math.round((ipinfo?.cache_ttl_seconds || 86400) / 3600)}h</b><span>失败缓存 / 单次首页上限</span><b>${Math.round((ipinfo?.failure_cache_ttl_seconds || 300) / 60)}min / ${ipinfo?.dashboard_lookup_limit || 128} 个 IP</b></div><p class="panel-footnote">key 只提交到后端，浏览器不会读取或回显完整 token；查询数据来自 <a href="https://ipinfo.io/lite" target="_blank" rel="noreferrer">IPinfo Lite</a>。</p></div><//><${Panel} eyebrow="Data boundary" title="运行边界" className="settings-note-panel"><div class="boundary-list"><div>${icon('shield', 17)}<span><b>事件流受保留策略限制</b><small>SQLite 是权威记录，JSONL 镜像会按保留天数与条数清理。</small></span></div><div>${icon('lock', 17)}<span><b>敏感字段脱敏</b><small>凭据、密码和 token 只保留 keyed fingerprint。</small></span></div><div>${icon('spark', 17)}<span><b>合成响应</b><small>不会运行模型、访问 URL 或连接供应商。</small></span></div></div><//></div></div>`
}
...
function App() {
  ...
  const saveIPInfo = useCallback(async (token) => {
    const ipinfo = await request('ipinfo-lite', { method: 'PUT', body: JSON.stringify({ token }) })
    setData((current) => ({ ...current, ipinfo }))
    showToast(token ? 'IPinfo Lite 配置已保存' : 'IPinfo Lite 已关闭')
