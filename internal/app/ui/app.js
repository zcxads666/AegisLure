...
const LIST_DEFAULTS = {
  observations: { page: 1, page_size: 10, q: '', product: '', min_score: '' },
  invocations: { page: 1, page_size: 10, q: '', product: '', level: '', auth: '', execution: '' },
  chains: { page: 1, page_size: 10, q: '' },
  indicators: { page: 1, page_size: 10, q: '', product: '', risk_level: '', sort: 'latest' },
}

const HONEYPOT_OPTIONS = [{ value: '', label: '全部蜜罐' }, ...Object.entries(PROFILE_LABELS).map(([value, label]) => ({ value, label }))]

const PAGE_SIZE_OPTIONS = [10, 20, 50, 100].map((value) => ({ value: String(value), label: `${value} 条` }))
...
function PaginationControls({ pagination, onPageChange, onPageSizeChange }) {
  ...
  return html`<div class="pagination-controls"><span>第 ${totalPages ? page : 0} / ${displayPages} 页 · 共 ${formatNumber(pagination.total || 0)} 条</span><div class="button-group">${onPageSizeChange ? html`<${Select} className="pagination-size-select" label="每页" value=${String(pageSize)} onChange=${(value) => onPageSizeChange(Number(value))} options=${PAGE_SIZE_OPTIONS} />` : null}<button class="outline-button" type="button" disabled=${!pagination.has_previous} onClick=${() => onPageChange(page - 1)}>上一页</button><button class="outline-button" type="button" disabled=${!pagination.has_next} onClick=${() => onPageChange(page + 1)}>下一页</button></div></div>`
}
...
function CountryDistribution({ items = [] }) {
  ...
  return html`<div class="country-list">${visible.map((item) => html`<div class="country-row" key=${item.key || item.name}><div class="country-row-head"><span><i class="country-bullet"></i>${item.name || '未知'}</span><b>${formatNumber(item.count)} <small>${item.percentage || 0}%</small></b></div><div class="share-progress"><i style=${{ width: `${Math.max(3, (Number(item.count || 0) / max) * 100)}%` }}></i></div></div>`)}</div><div class="panel-footnote">本地/保留和文档地址直接分类；公网地址通过 IPinfo API 查询，查询失败时显示“未知”。</div>`
}
...
function ServerObservationsPage({ events = [], pagination, onRefresh, onOpenEvent, onSearch, onPageChange, onPageSizeChange, onDelete, loading = false }) {
...
function ServerInvocationsPage({ invocations = [], pagination, onRefresh, onOpenEvent, onSearch, onPageChange, onPageSizeChange, onDelete, loading = false }) {
  ...
  const columns = [
    ...
    { label: '触发蜜罐', render: (row) => html`<${Badge} tone="neutral">${profileLabel(row.product)}<//>` },
  ...
  return html`<div class="page-stack"><${PageHeader} eyebrow="Synthetic execution trail" title="调用分析" description="查看每次模型调用尝试、触发蜜罐、鉴权结果、拒绝原因与合成执行阶段。" actions=${html`<${Button} icon="refresh" onClick=${onRefresh}>刷新调用<//>`} /><div class="callout callout-blue">${icon('spark', 18)}<div><b>合成执行边界</b><p>所有“已接受”调用只返回确定性的兼容响应，不会加载模型、执行 prompt 工具或连接供应商；成功和失败尝试都会提高风险分。</p></div></div><${Panel} className="table-panel" title="调用事件" action=${html`<span class="panel-meta">每页 ${pagination?.page_size || 10} 条 · 共 ${formatNumber(pagination?.total || 0)} 条</span>`}><${FilterBar} onReset=${reset}><label class="search-field">${icon('search', 17)}<input value=${query} onInput=${(event) => setQuery(event.target.value)} onKeyDown=${(event) => event.key === 'Enter' && apply()} placeholder="搜索调用 ID、模型、IP 或拒绝原因" /></label><${Select} label="触发蜜罐" value=${product} onChange=${setProduct} options=${HONEYPOT_OPTIONS} /><${Select} value=${level} onChange=${setLevel} options=${[{ value: '', label: '全部阶段' }, ...Object.entries(LEVEL_LABELS).map(([value, label]) => ({ value, label }))]} /><${Select} value=${auth} onChange=${setAuth} options=${[{ value: '', label: '全部鉴权' }, { value: 'valid_honey_key', label: '有效 honey key' }, { value: 'bypass_simulated', label: '模拟绕过' }, { value: 'missing', label: '缺失' }, { value: 'invalid', label: '无效' }]} /><${Select} value=${execution} onChange=${setExecution} options=${[{ value: '', label: '全部执行结果' }, { value: 'synthetic_accepted', label: '合成已接受' }, { value: 'synthetic_stream_completed', label: '合成流完成' }, { value: 'rejected_before_dispatch', label: '派发前拒绝' }]} /><${SearchButton} onClick=${apply} /></${FilterBar}><${DataTable} columns=${columns} rows=${invocations} onRowClick=${onOpenEvent} loading=${loading} loadingLabel="正在加载调用记录…" emptyTitle="还没有调用事件" emptyDescription="蜜罐记录到调用请求后，分析结果会显示在这里。" /><${PaginationControls} pagination=${pagination} onPageChange=${onPageChange} onPageSizeChange=${onPageSizeChange} /><//></div>`
}

function insightIdentityLabel(value) {
  if (value === 'account') return '账户跨 IP'
  if (value === 'key') return 'Key 跨 IP'
  if (value === 'identity') return '身份跨 IP'
  if (value === 'detection') return '前端检测'
  return value || '洞察'
}

function insightFindingLabel(value) {
  if (value === 'dns_region') return '地区不一致'
  if (value === 'webrtc_ip') return 'WebRTC 公网 IP 不一致'
  if (value === 'detection_consistent') return '未发现不一致'
  if (value === 'detection_indeterminate') return '证据不足，无法判断'
  return value || '检测结果'
}

function frontendCheckStatusLabel(value) {
  const labels = { disabled: '未启用', unknown: '信息不足', match: '一致', mismatch: '不一致', no_public_candidate: '无公网候选', source_ip_unavailable: '来源 IP 不可用', source_ip_match: '与来源 IP 一致', public_ip_mismatch: '公网 IP 不一致' }
  return labels[value] || value || '未知'
}

function ServerChainsPage({ chains = [], pagination, onRefresh, onOpenEvent, onSearch, onPageChange, onPageSizeChange, onDelete, loading = false }) {
  const [query, setQuery] = useState('')
  ...
  return html`<div class="page-stack"><${PageHeader} eyebrow="Information intelligence" title="信息洞察" description="按唯一 IP 对聚合账户或 API key 的创建与跨 IP 使用；同时记录成功登录后的地区一致性与 WebRTC 地址检测结果。" actions=${html`<${Button} icon="refresh" onClick=${onRefresh}>刷新洞察<//>`} /><div class="callout callout-blue">${icon('shield', 18)}<div><b>判定边界</b><p>只有“创建 IP”与“使用 IP”不同才生成跨 IP 洞察；同一 IP 对关联的多个账户和 Key 会合并为一条且保留全部证据。升级前身份若缺少创建证据，则至少观察到两个使用 IP 才展示并明确标记。New API root 账户组排除，但 root 创建的 key 仍保留。</p></div></div><${Panel} className="table-panel" title="洞察筛选" action=${html`<span class="panel-meta">每页 ${pagination?.page_size || 10} 条 · 共 ${formatNumber(pagination?.total || 0)} 条</span>`}><${FilterBar} onReset=${reset}><label class="search-field">${icon('search', 17)}<input value=${query} onInput=${(event) => setQuery(event.target.value)} onKeyDown=${(event) => event.key === 'Enter' && apply()} placeholder="搜索账户 / key 指纹、IP、地区或检测类型" /></label><${SearchButton} onClick=${apply} /></${FilterBar}></${Panel}>${loading ? html`<${LoadingState} label="正在加载信息洞察…" />` : chains?.length ? html`<div class="chain-grid">${chains.map((item) => html`<article class="chain-card insight-card" key=${item.id}><header class="chain-card-header"><div class="chain-id"><span class="chain-mark">${icon(item.identity_type === 'detection' ? 'shield' : 'route', 17)}</span><div><b>${insightIdentityLabel(item.identity_type)}</b><small>${item.identity_count > 1 ? `${item.identity_count} 个关联身份` : shortValue(item.subject_fingerprint, 24)}${item.related_account ? ` · 账户 ${shortValue(item.related_account, 14)}` : ''}</small></div></div><${RiskBadge} score=${item.score} /></header><div class="chain-meta"><span>${icon('clock', 14)}${item.event_count || 0} 个事件</span><span>${icon('clock', 14)}最近 ${formatTime(item.last_seen, true)}</span><${Badge} tone=${item.identity_type === 'detection' ? 'warning' : 'blue'}>${insightIdentityLabel(item.identity_type)}<//>${item.creation_evidence_missing ? html`<${Badge} tone="warning">历史创建证据缺失<//>` : null}${item.different_ip_count ? html`<${Badge} tone="danger">${item.different_ip_count} 个唯一 IP 对<//>` : null}</div>${item.identity_type !== 'detection' ? html`<div class="insight-ip-grid"><div><span>创建 IP</span><code>${(item.creation_ips || []).join(' · ') || (item.creation_evidence_missing ? '历史版本未记录' : '—')}</code></div><div><span>使用 IP</span><code>${(item.usage_ips || []).join(' · ') || '—'}</code></div></div>` : null}${item.detection_findings?.length ? html`<div class="insight-findings">${item.detection_findings.map((finding, index) => html`<div class="insight-finding" key=${finding.event_id || index}><${Badge} tone=${finding.kind === 'detection_consistent' ? 'success' : finding.kind === 'detection_indeterminate' ? 'neutral' : 'warning'}>${insightFindingLabel(finding.kind)}<//><span>${finding.inferred_ip || finding.server_ip || '—'} · ${finding.inferred_region || finding.ip_region || '地区未知'}</span><small>地区 ${frontendCheckStatusLabel(finding.region_status)} · WebRTC ${frontendCheckStatusLabel(finding.webrtc_status)} · 访客 ${finding.visitor_region || '未知'} · ${finding.visitor_timezone || '时区未知'} · ${formatTime(finding.observed_at, true)}</small></div>`)}</div>` : null}<div class="chain-line">${(item.events || []).slice(-5).map((event, index, visible) => html`<button type="button" class="chain-event" onClick=${() => onOpenEvent(event)}><i class=${cn('chain-dot', event.score >= 60 && 'is-risk')}></i><span><b>${event.event_type || semanticRoute(event)}</b><small>${formatTime(event.observed_at)} · ${event.status || '—'} · ${rawRoute(event)}</small></span>${index === visible.length - 1 ? html`<em>latest</em>` : null}</button>`)}</div><div class="chain-card-actions"><button class="chain-expand" type="button" onClick=${() => setExpanded(expanded === item.id ? null : item.id)}>${expanded === item.id ? '收起详情' : '查看完整证据'} ${icon('chevron', 14)}</button><${DeleteButton} onClick=${() => onDelete(item.id)} /></div>${expanded === item.id ? html`<div class="chain-expanded"><pre class="json-view">${JSON.stringify(item, null, 2)}</pre></div>` : null}</article>`)}</div>` : html`<${Panel}><${EmptyState} icon="shield" title="还没有信息洞察" description="当账户或 key 出现跨 IP 使用，或前端检测完成一次地区 / WebRTC 判定时，这里会显示证据卡片。" /><//>`}<${PaginationControls} pagination=${pagination} onPageChange=${onPageChange} onPageSizeChange=${onPageSizeChange} /></div>`
}

function ServerIndicatorsPage({ indicators = [], pagination, onRefresh, onOpenIndicator, onSearch, onPageChange, onPageSizeChange, onDelete, loading = false }) {
  const [query, setQuery] = useState('')
  const [product, setProduct] = useState('')
  const [riskLevel, setRiskLevel] = useState('')
  const [sortMode, setSortMode] = useState('latest')
  const apply = () => onSearch({ page: 1, q: query, product, risk_level: riskLevel, sort: sortMode })
  const reset = () => { setQuery(''); setProduct(''); setRiskLevel(''); setSortMode('latest'); onSearch({ page: 1, q: '', product: '', risk_level: '', sort: 'latest' }) }
  const exportIndicators = (format, scope = 'filtered') => {
    ...
    const link = document.createElement('a'); link.href = `${apiPath('indicators')}?${params}`; link.download = `aegislure-indicators-${scope === 'all' ? 'all' : 'filtered'}.${format}`; document.body.appendChild(link); link.click(); link.remove()
  }
  const columns = [
    ...
    { label: '触发蜜罐', render: (row) => html`<div class="button-group">${(row.products || []).map((value) => html`<${Badge} tone="neutral">${profileLabel(value)}<//>`)}</div>` },
  ...
  return html`<div class="page-stack"><${PageHeader} eyebrow="Risk intelligence" title="IP 情报" description="显示每个 IP 触发过的蜜罐；可按蜜罐、风险等级筛选，并按最近出现时间或风险分排序。" actions=${html`<div class="button-group"><${Button} icon="download" size="sm" onClick=${() => exportIndicators('csv', 'all')}>导出 CSV（全量）<//><${Button} icon="refresh" size="sm" onClick=${onRefresh}>刷新<//></div>`} /><${Panel} className="table-panel" title="指标列表" action=${html`<span class="panel-meta">每页 ${pagination?.page_size || 10} 条 · 共 ${formatNumber(pagination?.total || 0)} 个指标</span>`}><${FilterBar} onReset=${reset}><label class="search-field">${icon('search', 17)}<input value=${query} onInput=${(event) => setQuery(event.target.value)} onKeyDown=${(event) => event.key === 'Enter' && apply()} placeholder="搜索 IP（完整或部分）" /></label><${Select} label="触发蜜罐" value=${product} onChange=${setProduct} options=${HONEYPOT_OPTIONS} /><${Select} label="风险等级" value=${riskLevel} onChange=${setRiskLevel} options=${INDICATOR_RISK_OPTIONS} /><${Select} label="排序方式" value=${sortMode} onChange=${setSortMode} options=${INDICATOR_SORT_OPTIONS} /><${SearchButton} onClick=${apply} /><//><div class="indicator-tools"><div class="indicator-order-note"><span>蜜罐：${product ? profileLabel(product) : '全部蜜罐'}</span><span>筛选：${selectedRiskLabel}</span><span>排序：${selectedSortLabel}</span></div><div class="button-group"><button class="outline-button" type="button" onClick=${() => exportIndicators('plain', 'filtered')}>导出纯文本（当前筛选）</button><button class="outline-button" type="button" onClick=${() => exportIndicators('csv', 'filtered')}>下载 CSV（当前筛选）</button></div></div><${DataTable} columns=${columns} rows=${indicators} onRowClick=${onOpenIndicator} loading=${loading} loadingLabel="正在加载 IP 指标…" emptyTitle="还没有 IP 指标" emptyDescription="当观测到公开蜜罐端点请求后，风险聚合会出现在这里。" /><${PaginationControls} pagination=${pagination} onPageChange=${onPageChange} onPageSizeChange=${onPageSizeChange} /><//><p class="page-note">地理信息由当前 GeoIP provider 查询；provider 切换后会重新查询历史 IP。删除 IP 只写入事件 tombstone，不修改权威原始事件。</p></div>`
}

function modelListText(values) { return Array.isArray(values) ? values.join(', ') : '' }

function parseModelList(value) { return value.split(',').map((item) => item.trim()).filter(Boolean) }

function emptyInstanceModel(product, index) {
  const selfHosted = !['new-api', 'sub2api'].includes(product)
  return {
    id: `draft-${Date.now()}-${index}`, public_model_id: '', display_name: '', provider: selfHosted ? 'local' : 'provider',
    origin: selfHosted ? 'open' : 'closed', capabilities: ['chat'], api_families: ['openai'], visibility: ['guest', 'user'],
    auth_requirement: product === 'ollama' ? 'none' : 'api_key', virtual_context_tokens: 0, virtual_price_profile: '',
    status: 'active', aliases: [], response_template_set: '', architecture: '', families: [], parameter_size: '',
    quantization_level: '', approx_size: 0,
  }
}

function InstanceModelsModal({ instance, onClose, onSaved }) {
  ...
  useEffect(() => {
    let active = true
    request(`instances/${instance.product}/models`).then((result) => { if (!active) return; setState(result); setModels(result.models || []); setLoading(false) }).catch((error) => { if (!active) return; setMessage(error.message || '模型配置读取失败'); setMessageError(true); setLoading(false) })
    return () => { active = false }
  }, [instance.product])
  const update = (index, field, value) => setModels((current) => current.map((item, itemIndex) => itemIndex === index ? { ...item, [field]: value } : item))
  const addModel = () => setModels((current) => [...current, emptyInstanceModel(instance.product, current.length)])
  const removeModel = (index) => setModels((current) => current.filter((_, itemIndex) => itemIndex !== index))
  const save = async () => {
    setSaving(true); setMessage(''); setMessageError(false)
    try {
      const result = await request(`instances/${instance.product}/models`, { method: 'PUT', body: JSON.stringify({ models: models.map((item) => ({ ...item, virtual_context_tokens: Number(item.virtual_context_tokens || 0), approx_size: Number(item.approx_size || 0) })) }) })
      setState(result); setModels(result.models || []); setMessage(`已统一更新 ${result.model_count || 0} 个模型，相关公开页面与协议将在新会话中使用此目录。`); await onSaved()
    } catch (error) { setMessage(error.message || '模型配置保存失败'); setMessageError(true) } finally { setSaving(false) }
  }
  const restore = async () => {
    if (!window.confirm(`确认恢复 ${profileLabel(instance.product)} 的默认模型目录吗？`)) return
    setSaving(true); setMessage(''); setMessageError(false)
    try { const result = await request(`instances/${instance.product}/models`, { method: 'PUT', body: JSON.stringify({ restore_default: true }) }); setState(result); setModels(result.models || []); setMessage('已恢复默认模型目录。'); await onSaved() }
    catch (error) { setMessage(error.message || '恢复默认模型目录失败'); setMessageError(true) } finally { setSaving(false) }
  }
  return html`<${Modal} title=${`${profileLabel(instance.product)} · 模型详细设置`} onClose=${onClose} wide=${true}>${loading ? html`<${LoadingState} label="正在汇总模型配置…" />` : html`<div class="model-settings"><div class="model-settings-summary"><div><span>当前目录</span><strong>${state?.custom ? '实例自定义' : '系统默认'} · ${models.length} 个模型</strong><small>${state?.pack_id || 'compiled-default'} · ${state?.revision || 'unknown'}</small></div><${Badge} tone=${state?.custom ? 'warning' : 'success'}>${state?.custom ? 'Custom' : 'Default'}<//></div><div class="model-surface-box"><b>统一影响范围</b><div class="chip-list">${(state?.surfaces || []).map((surface) => html`<span class="chip">${surface}</span>`)}</div></div><div class="model-editor-list">${models.map((item, index) => html`<details class="model-editor-card" key=${item.id || index} open=${index === 0}><summary><span><b>${item.display_name || item.public_model_id || `未命名模型 ${index + 1}`}</b><code>${item.public_model_id || '等待填写公开模型 ID'}</code></span><div><${Badge} tone=${item.status === 'disabled' ? 'neutral' : 'blue'}>${item.status || 'active'}<//><em>${index + 1}</em></div></summary><div class="model-editor-body"><div class="model-form-grid"><${TextInput} label="公开模型 ID" value=${item.public_model_id || ''} placeholder="例如 provider/model-name" onInput=${(value) => update(index, 'public_model_id', value)} maxLength="256" required=${true} /><${TextInput} label="展示名称" value=${item.display_name || ''} placeholder="模型在人类可读页面中的名称" onInput=${(value) => update(index, 'display_name', value)} maxLength="256" required=${true} /><${TextInput} label="Provider" value=${item.provider || ''} placeholder="openai / qwen / local" onInput=${(value) => update(index, 'provider', value)} maxLength="128" required=${true} /><${Select} label="来源类型" value=${item.origin || 'open'} onChange=${(value) => update(index, 'origin', value)} options=${[{ value: 'open', label: 'open · 开放模型' }, { value: 'closed', label: 'closed · 闭源模型' }]} /><${Select} label="状态" value=${item.status || 'active'} onChange=${(value) => update(index, 'status', value)} options=${[{ value: 'active', label: 'active · 展示' }, { value: 'disabled', label: 'disabled · 隐藏' }]} /><${Select} label="鉴权要求" value=${item.auth_requirement || ''} onChange=${(value) => update(index, 'auth_requirement', value)} options=${[{ value: '', label: '跟随蜜罐默认' }, { value: 'none', label: 'none · 无需 key' }, { value: 'api_key', label: 'api_key · API key' }, { value: 'honey_key', label: 'honey_key · 蜜罐 key' }]} /><${TextInput} label="能力（逗号分隔）" value=${modelListText(item.capabilities)} placeholder="chat, vision, tools" onInput=${(value) => update(index, 'capabilities', parseModelList(value))} /><${TextInput} label="别名（逗号分隔）" value=${modelListText(item.aliases)} placeholder="兼容调用名或 Ollama 展示名" onInput=${(value) => update(index, 'aliases', parseModelList(value))} /><${TextInput} label="API family（逗号分隔）" value=${modelListText(item.api_families)} placeholder="openai, responses, anthropic" onInput=${(value) => update(index, 'api_families', parseModelList(value))} /><${TextInput} label="可见范围（逗号分隔）" value=${modelListText(item.visibility)} placeholder="guest, user" onInput=${(value) => update(index, 'visibility', parseModelList(value))} /><${TextInput} label="虚拟上下文长度" type="number" min="0" max="10000000" value=${item.virtual_context_tokens || 0} onInput=${(value) => update(index, 'virtual_context_tokens', value)} /><${TextInput} label="虚拟价格档案" value=${item.virtual_price_profile || ''} placeholder="例如 standard" onInput=${(value) => update(index, 'virtual_price_profile', value)} maxLength="128" /><${TextInput} label="响应模板集" value=${item.response_template_set || ''} placeholder="例如 openai / anthropic" onInput=${(value) => update(index, 'response_template_set', value)} maxLength="128" />${state?.self_hosted_fields ? html`<${TextInput} label="模型架构" value=${item.architecture || ''} placeholder="llama / qwen35moe" onInput=${(value) => update(index, 'architecture', value)} maxLength="128" /><${TextInput} label="模型家族（逗号分隔）" value=${modelListText(item.families)} placeholder="llama, qwen" onInput=${(value) => update(index, 'families', parseModelList(value))} /><${TextInput} label="参数规模" value=${item.parameter_size || ''} placeholder="例如 35B" onInput=${(value) => update(index, 'parameter_size', value)} maxLength="128" /><${TextInput} label="量化等级" value=${item.quantization_level || ''} placeholder="例如 Q4_K_M" onInput=${(value) => update(index, 'quantization_level', value)} maxLength="128" /><${TextInput} label="近似大小（字节）" type="number" min="0" value=${item.approx_size || 0} onInput=${(value) => update(index, 'approx_size', value)} />` : null}</div><div class="model-editor-actions"><button class="text-button danger-text" type="button" onClick=${() => removeModel(index)} disabled=${models.length <= 1 || saving}>删除这个模型</button></div></div></details>`)}</div><div class="model-settings-actions"><${Button} variant="secondary" icon="plus" onClick=${addModel} disabled=${saving || models.length >= (state?.max_models || 64)}>添加模型<//><div class="button-group"><${Button} variant="ghost" onClick=${restore} disabled=${saving || !state?.custom}>恢复默认<//><${Button} variant="primary" onClick=${save} disabled=${saving || models.length === 0}>${saving ? '保存中…' : '保存全部模型'}<//></div></div>${message ? html`<div class=${cn('form-message', messageError && 'error', !messageError && 'success')}>${message}</div>` : null}<p class="panel-footnote">此处是该蜜罐的统一模型目录。保存后会结束该蜜罐旧的匿名会话，使模型列表、详情页、调用校验和响应字段从下一次请求起保持一致。</p></div>`}<//>`
}

function InstanceCard({ instance, busy, onAction, onConfigureModels }) {
  const running = instance.state === 'running'
  const preview = (instance.model_preview || []).slice(0, 3)
  return html`<article class=${cn('instance-card', running && 'is-running')}><div class="instance-card-head"><div class="instance-logo">${profileLabel(instance.product).slice(0, 1)}</div><div class="instance-title"><div><h3>${profileLabel(instance.product)}</h3><${StatusBadge} state=${instance.state} /></div><p>${instance.profile_id || 'profile'} · ${instance.version || 'version unknown'}</p></div><${Toggle} checked=${running} label=${`${profileLabel(instance.product)} 开关`} onChange=${(next) => onAction(instance, next ? 'start' : 'stop')} /></div><div class="instance-details"><div><span>监听端口</span><b>${instance.port || '—'}</b></div><div><span>场景</span><b>${instance.scenario || 'default'}</b></div><div><span>端点</span><code>${instance.endpoint || '—'}</code></div></div><button class="instance-model-summary" type="button" onClick=${() => onConfigureModels(instance)}><span><b>模型详细设置</b><small>${instance.model_count || 0} 个模型 · 统一配置所有展示入口</small></span>${icon('chevron', 16)}</button><div class="instance-model-preview">${preview.map((name) => html`<code>${name}</code>`)}${(instance.model_count || 0) > preview.length ? html`<small>+${instance.model_count - preview.length}</small>` : null}</div><div class="instance-card-foot"><span>${instance.synthetic_only ? html`<span class="instance-boundary">安全合成边界</span>` : '—'}</span><div class="button-group"><${Button} size="sm" variant="ghost" icon="refresh" onClick=${() => onAction(instance, 'restart')} disabled=${busy}>重启<//>${running ? html`<${Button} size="sm" variant="danger-ghost" icon="pause" onClick=${() => onAction(instance, 'stop')} disabled=${busy}>停止<//>` : html`<${Button} size="sm" variant="primary-soft" icon="play" onClick=${() => onAction(instance, 'start')} disabled=${busy}>启动<//>`}</div></div></article>`
}
...
function InstancesPage({ instances, onRefresh, onAction, busy }) {
  ...
  return html`
    <div class="page-stack">
      <${PageHeader} title="蜜罐实例" description="按协议启停公开端点，并为每个蜜罐独立设置对外显示的模型名称。" actions=${html`<div class="button-group"><${Button} variant="secondary" icon="play" onClick=${() => onAction({ product: '__all__' }, 'start-all')} disabled=${busy}>启动全部<//><${Button} icon="refresh" onClick=${onRefresh}>刷新状态<//></div>`} />
      ...
      <div class="instance-grid">${instances?.map((instance) => html`<${InstanceCard} instance=${instance} busy=${busy} onAction=${onAction} onConfigureModels=${setModelEditor} />`)}</div>
      <p class="page-note">停止实例会关闭对应公开监听器并更新运行配置；管理端与公开监听器彼此独立。</p>
    </div>${modelEditor ? html`<${InstanceModelsModal} instance=${modelEditor} onClose=${() => setModelEditor(null)} onSaved=${onRefresh} />` : null}
  `
}
...
function PacksPage({ packs, policies, onRefresh }) {
  ...
  useEffect(() => {
    ...
    const query = new URLSearchParams({ page: String(rulePage), page_size: String(rulePageSize) })
    ...
    return () => { active = false }
  }, [selectedPackID, rulePage, rulePageSize, ruleQuery])
  ...
  const togglePolicy = async (policy) => {
    ...
    try {
      ...
      setPolicyMessage(`${policy.provider} 登录/注册页入口已${policy.enabled ? '关闭' : '开启'}。`)
      await onRefresh()
    } catch (error) {
...
function SettingsPage({ username, ipinfo, frontendDetection, ipListAPI, onRotateEntry, onSaveIPInfo, onSaveFrontendDetection, onSaveIPListAPI, onRotateIPListAPI }) {
  ...
  const apiProviderName = 'IPinfo API'
  const detectionConfig = frontendDetection?.config || frontendDetection || {}
  useEffect(() => { setProvider('ipinfo_api') }, [ipinfo?.provider])
  const rotate = () => { if (window.confirm('轮换后当前管理入口会立即失效，需要从配置或启动输出获取新路径。继续吗？')) onRotateEntry() }
  const updateIPListAPI = async (enabled) => {
    setApiBusy(true); setApiMessage('')
    try {
      const result = await onSaveIPListAPI(Boolean(enabled))
      if (result?.api_key) setApiKey(result.api_key)
      setApiMessage(result?.api_key ? '接口已开启，新 key 只在本次显示，请立即复制。' : `接口已${enabled ? '开启' : '关闭'}。`)
    } catch (error) { setApiMessage(error.message || 'IP 列表接口设置保存失败。') } finally { setApiBusy(false) }
  }
  const rotateIPListAPI = async () => {
    if (!window.confirm('轮换后旧 API key 会立即失效，继续吗？')) return
    setApiBusy(true); setApiMessage('')
    try {
      const result = await onRotateIPListAPI()
      if (result?.api_key) setApiKey(result.api_key)
      setApiMessage('API key 已轮换，旧 key 已失效；请立即复制新 key。')
    } catch (error) { setApiMessage(error.message || 'API key 轮换失败。') } finally { setApiBusy(false) }
  }
  const copyIPListAPIKey = async () => {
    if (!apiKey) return
    try {
      if (!navigator.clipboard?.writeText) throw new Error('当前浏览器不支持自动复制，请手动复制。')
      await navigator.clipboard.writeText(apiKey)
      setApiMessage('完整 API key 已复制。')
    } catch (error) { setApiMessage(error.message || 'API key 复制失败，请手动复制。') }
  }
  ...
  const save = async (nextProvider = provider, nextToken = token, includeToken = false) => {
    ...
    try {
      ...
      setToken('')
      setMessage(includeToken && nextToken ? `${apiProviderName} key 已保存，并已验证 8.8.8.8 可正常获取；完整 key 不会回显。` : includeToken ? `${apiProviderName} key 已清除。` : ipinfoConfigured ? `${apiProviderName} 已保持启用。` : `请输入 ${apiProviderName} key 后启用。`)
    } catch (error) {
      setMessage(error.message || '地理情报设置保存失败。'); setMessageTone('error')
    } finally { setBusy(false) }
  }
  const providerOptions = [{ value: 'ipinfo_api', label: 'IPinfo API（City + ASN）' }]
  const statusTone = configured ? 'success' : 'warning'
  const apiEndpoint = ipListAPI?.endpoint || `${window.location.origin}${BASE}api`
  const apiKeyDisplay = apiKey || ipListAPI?.key_masked || '尚未生成（开启时自动生成）'
  const apiStatusTone = ipListAPI?.enabled ? 'success' : 'neutral'
  const providerPanel = html`<div class="geoip-provider-panel"><${TextInput} label="IPinfo API Token / Key" type="password" value=${token} onInput=${setToken} placeholder=${ipinfoConfigured ? `当前已配置（${ipinfo?.masked_token || '已隐藏'}），输入新 key 可替换` : '粘贴 IPinfo API key（不会返回原文）'} autoComplete="new-password" maxLength=${256} /><div class="ipinfo-facts"><span>接口</span><code>${ipinfo?.endpoint || 'https://ipinfo.io/{ip}'}</code><span>超时 / 成功缓存</span><b>${ipinfo?.timeout_seconds || 2}s / ${Math.round((ipinfo?.cache_ttl_seconds || 86400) / 3600)}h</b><span>失败缓存 / 单次首页上限</span><b>${Math.round((ipinfo?.failure_cache_ttl_seconds || 300) / 60)}min / ${ipinfo?.dashboard_lookup_limit || 128} 个 IP</b></div><p class="panel-footnote">key 只提交到后端，浏览器不会读取或回显完整 token；查询数据来自 <a href="https://ipinfo.io/developers/ip-to-geolocation-database" target="_blank" rel="noreferrer">IPinfo IP Geolocation API</a>。保存 key 时会先验证 <code>8.8.8.8</code>。</p></div>`
  return html`<div class="page-stack"><${PageHeader} eyebrow="Workspace settings" title="管理设置" description="管理离线恢复凭据、管理端入口、IP 地理情报来源、IP 列表接口和登录后的前端检测。" /><div class="settings-grid"><${Panel} eyebrow="Account security" title="离线恢复" className="settings-form-panel"><div class="security-card"><div class="security-card-icon">${icon('key', 22)}</div><h3>${username || 'owner'} · 恢复凭据</h3><p>管理端不提供在线改密。请使用一次性 recovery code，或在服务器本机通过 hpctl 生成 rescue code 后完成恢复。</p><span class="instance-boundary">恢复码成功使用后立即失效</span></div><//><${Panel} eyebrow="Admin endpoint" title="管理入口"><div class="security-card"><div class="security-card-icon">${icon('key', 22)}</div><h3>随机隐藏路径</h3><p>当前入口路径不会在 API 响应中返回。轮换后会使现有管理会话失效，页面会跳转到新的入口。</p><button class="button button-danger-soft button-full" type="button" onClick=${rotate}>${icon('refresh', 16)}轮换管理入口</button></div><//><${Panel} eyebrow="IP intelligence" title="IP 地理情报" className="settings-form-panel"><div class="security-card ipinfo-settings-card"><div class="settings-card-heading"><div class="security-card-icon">${icon('globe', 22)}</div><${Badge} tone=${statusTone}>${configured ? '已就绪' : '未就绪'}<//></div><h3>选择公网 IP 查询来源</h3><p>本地、保留和文档地址直接本地识别；公网地址统一通过 IPinfo API 查询，查询失败时回退为“未知”。</p><${Select} label="查询 provider" value=${provider} onChange=${setProvider} options=${providerOptions} /><div class="button-group geoip-save-row"><${Button} variant="primary" size="sm" onClick=${() => save(provider, token, Boolean(token.trim()))} disabled=${busy || (!token.trim() && !ipinfoConfigured)}>${busy ? '保存中…' : '保存查询设置'}<//>${ipinfoConfigured ? html`<${Button} variant="ghost" size="sm" onClick=${() => save('ipinfo_api', '', true)} disabled=${busy}>清除并停用 key<//>` : null}</div>${providerPanel}${message ? html`<div class=${cn('form-message', messageTone === 'error' && 'error', messageTone !== 'error' && 'success')}>${message}</div>` : null}</div><//><${Panel} eyebrow="IP list API" title="IP 列表接口" className="settings-form-panel"><div class="security-card ip-list-api-card"><div class="settings-card-heading"><div class="security-card-icon">${icon('shield', 22)}</div><${Badge} tone=${apiStatusTone}>${ipListAPI?.enabled ? '已开启' : '已关闭'}<//></div><h3>通过后台地址提供风险 IP 列表</h3><p>接口位于当前后台隐藏地址的 <code>/api</code> 路径。默认返回全量 IP；也可以按最近天数、自然月和低 / 中 / 高三档风险筛选。</p><div class="api-toggle-row"><span><b>启用对外查询接口</b><small>关闭后接口返回 404；启用时首次自动生成 key。</small></span><${Toggle} checked=${Boolean(ipListAPI?.enabled)} onChange=${updateIPListAPI} disabled=${apiBusy} label="切换 IP 列表 API" /></div><div class="ipinfo-facts api-list-facts"><span>接口地址</span><code>${apiEndpoint}</code><span>请求方式</span><b>GET</b><span>鉴权方式</span><code>Authorization: Bearer &lt;API key&gt;</code><span>筛选示例</span><code>?days=7&amp;risk=high · ?month=2026-09</code></div><div class="api-key-panel"><div><span>当前 key</span><code class=${cn('api-key-display', apiKey && 'is-revealed')}>${apiKeyDisplay}</code></div><div class="button-group"><${Button} variant="ghost" size="sm" onClick=${copyIPListAPIKey} disabled=${!apiKey}>${icon('copy', 15)}复制本次 key<//><${Button} variant="secondary" size="sm" onClick=${rotateIPListAPI} disabled=${apiBusy}>${icon('refresh', 15)}轮换 key<//></div></div>${apiKey ? html`<div class="notice notice-warning">${ipListAPI?.key_rotated_at ? '完整 key 只在生成/轮换响应中显示。' : ''} 请立即保存；页面刷新后只保留遮罩前缀。</div>` : null}${apiMessage ? html`<div class="form-message ${apiMessage.includes('失败') || apiMessage.includes('不支持') ? 'error' : 'success'}">${apiMessage}</div>` : null}<p class="panel-footnote">不会把完整 key 保存到配置或状态文件，只保存不可逆指纹；轮换会立即使旧 key 失效。参数不带筛选时返回全部可见 IP 指标。</p></div><//><${Panel} eyebrow="Frontend detection" title="登录后前端检测" className="settings-form-panel"><div class="security-card"><div class="settings-card-heading"><div class="security-card-icon">${icon('shield', 22)}</div><${Badge} tone=${detectionConfig.dns_leak_detection || detectionConfig.webrtc_ip_detection ? 'success' : 'neutral'}>${detectionConfig.dns_leak_detection || detectionConfig.webrtc_ip_detection ? '已开启' : '未开启'}<//></div><p>仅在 New API / Sub2API 前端成功登录后执行。地区判定使用浏览器语言地区与来源 IP 的 GeoIP 国家码；时区仅作上下文记录。WebRTC 只比较可判定的公网候选 IP，不采集凭据或页面内容。没有公网候选时会显示“证据不足，无法判断”。</p><div class="boundary-list"><div><${Toggle} checked=${Boolean(detectionConfig.dns_leak_detection)} onChange=${(next) => updateDetection('dns_leak_detection', next)} disabled=${detectionBusy !== ''} label="切换地区一致性检测" /><span><b>地区一致性</b><small>浏览器语言地区与来源 IP GeoIP 国家码不一致时记录线索。</small></span></div><div><${Toggle} checked=${Boolean(detectionConfig.webrtc_ip_detection)} onChange=${(next) => updateDetection('webrtc_ip_detection', next)} disabled=${detectionBusy !== ''} label="切换 WebRTC 公网候选 IP 检测" /><span><b>WebRTC 公网候选 IP</b><small>比较可判定的公网候选地址与请求来源；无候选时标记为无法判断。</small></span></div></div>${detectionMessage ? html`<div class="form-message ${detectionMessage.includes('失败') ? 'error' : 'success'}">${detectionMessage}</div>` : null}</div><//><${Panel} eyebrow="Data boundary" title="运行边界" className="settings-note-panel"><div class="boundary-list"><div>${icon('shield', 17)}<span><b>事件流受保留策略限制</b><small>SQLite 是权威记录，JSONL 镜像会按保留天数与条数清理。</small></span></div><div>${icon('lock', 17)}<span><b>敏感字段脱敏</b><small>凭据、密码和 token 只保留 keyed fingerprint。</small></span></div><div>${icon('spark', 17)}<span><b>合成响应</b><small>不会运行模型、访问 URL 或连接供应商。</small></span></div></div><//></div></div>`
}
...
function App() {
  ...
  const loadRoute = useCallback(async (target = route, quiet = false, overrideParams = null) => {
    ...
    try {
      ...
      else if (target === 'observations') { const params = overrideParams || listParamsRef.current.observations; const query = new URLSearchParams({ page: String(params.page || 1), page_size: String(params.page_size || 10), projection: 'summary' }); if (params.q) query.set('q', params.q); if (params.product) query.set('product', params.product); if (params.min_score !== '' && params.min_score != null) query.set('min_score', String(params.min_score)); loaded = await request(`events?${query}`, options); if (!current()) return null; setData((value) => ({ ...value, events: loaded.events || [], pagination: { ...value.pagination, observations: responsePagination(loaded) } })) }
      else if (target === 'invocations') { const params = overrideParams || listParamsRef.current.invocations; const query = new URLSearchParams({ page: String(params.page || 1), page_size: String(params.page_size || 10), projection: 'summary' }); if (params.q) query.set('q', params.q); if (params.product) query.set('product', params.product); if (params.level) query.set('level', params.level); if (params.auth) query.set('auth', params.auth); if (params.execution) query.set('execution', params.execution); loaded = await request(`invocations?${query}`, options); if (!current()) return null; setData((value) => ({ ...value, invocations: loaded.invocations || [], pagination: { ...value.pagination, invocations: responsePagination(loaded) } })) }
      else if (target === 'chains') { const params = overrideParams || listParamsRef.current.chains; const query = new URLSearchParams({ page: String(params.page || 1), page_size: String(params.page_size || 10), projection: 'summary' }); if (params.q) query.set('q', params.q); loaded = await request(`insights?${query}`, options); if (!current()) return null; setData((value) => ({ ...value, chains: loaded.insights || loaded.items || [], pagination: { ...value.pagination, chains: responsePagination(loaded) } })) }
      else if (target === 'indicators') { const params = overrideParams || listParamsRef.current.indicators; const query = new URLSearchParams({ page: String(params.page || 1), page_size: String(params.page_size || 10) }); if (params.q) query.set('q', params.q); if (params.product) query.set('product', params.product); if (params.risk_level) query.set('risk_level', params.risk_level); if (params.sort) query.set('sort', params.sort); if (params.min_score !== '' && params.min_score != null) query.set('min_score', String(params.min_score)); loaded = await request(`indicators?${query}`, options); if (!current()) return null; setData((value) => ({ ...value, indicators: loaded.items || [], pagination: { ...value.pagination, indicators: responsePagination(loaded) } })) }
      ...
      else if (target === 'settings') { const [ipinfo, ipListAPI, frontendDetection] = await Promise.all([request('ipinfo', options), request('ip-list-api', options), request('frontend-detection', options)]); if (!current()) return null; setData((value) => ({ ...value, ipinfo, ipListAPI, frontendDetection })) }
  ...
  const saveIPInfo = useCallback(async (provider, token, includeToken = false) => {
    ...
    const ipinfo = await request('ipinfo', { method: 'PUT', body: JSON.stringify(payload) })
    setData((current) => ({ ...current, ipinfo }))
    showToast(includeToken && token ? 'IPinfo API key 已保存并验证 8.8.8.8 可用' : includeToken ? 'IPinfo API key 已清除' : 'IPinfo API 设置已更新')
  ...
  const saveIPListAPI = useCallback(async (enabled) => {
    const ipListAPI = await request('ip-list-api', { method: 'PUT', body: JSON.stringify({ enabled: Boolean(enabled) }) })
    setData((current) => ({ ...current, ipListAPI }))
    return ipListAPI
  }, [])
  const rotateIPListAPI = useCallback(async () => {
    const ipListAPI = await request('ip-list-api/key:rotate', { method: 'POST' })
    setData((current) => ({ ...current, ipListAPI }))
    return ipListAPI
  }, [])
  ...
  const page = route === 'observations' ? html`<${ObservationsPage} events=${data.events} pagination=${data.pagination.observations} onRefresh=${() => loadRoute('observations')} onSearch=${(changes) => loadList('observations', changes)} onPageChange=${(page) => loadList('observations', { page })} onPageSizeChange=${(pageSize) => loadList('observations', { page: 1, page_size: pageSize })} onDelete=${(id) => deleteListItem('observations', id)} onOpenEvent=${openEvent} loading=${listLoading?.target === 'observations' && (listLoading.query || data.events.length === 0)} />` : route === 'invocations' ? html`<${InvocationsPage} invocations=${data.invocations} pagination=${data.pagination.invocations} onRefresh=${() => loadRoute('invocations')} onSearch=${(changes) => loadList('invocations', changes)} onPageChange=${(page) => loadList('invocations', { page })} onPageSizeChange=${(pageSize) => loadList('invocations', { page: 1, page_size: pageSize })} onDelete=${(id) => deleteListItem('invocations', id)} onOpenEvent=${openEvent} loading=${listLoading?.target === 'invocations' && (listLoading.query || data.invocations.length === 0)} />` : route === 'chains' ? html`<${ChainsPage} chains=${data.chains} pagination=${data.pagination.chains} onRefresh=${() => loadRoute('chains')} onSearch=${(changes) => loadList('chains', changes)} onPageChange=${(page) => loadList('chains', { page })} onPageSizeChange=${(pageSize) => loadList('chains', { page: 1, page_size: pageSize })} onDelete=${(id) => deleteListItem('chains', id)} onOpenEvent=${openEvent} loading=${listLoading?.target === 'chains' && (listLoading.query || data.chains.length === 0)} />` : route === 'indicators' ? html`<${IndicatorsPage} indicators=${data.indicators} pagination=${data.pagination.indicators} onRefresh=${() => loadRoute('indicators')} onSearch=${(changes) => loadList('indicators', changes)} onPageChange=${(page) => loadList('indicators', { page })} onPageSizeChange=${(pageSize) => loadList('indicators', { page: 1, page_size: pageSize })} onDelete=${(id) => deleteListItem('indicators', id)} onOpenIndicator=${openActor} loading=${listLoading?.target === 'indicators' && (listLoading.query || data.indicators.length === 0)} />` : route === 'instances' ? html`<${InstancesPage} instances=${data.instances} onRefresh=${() => loadRoute('instances')} onAction=${instanceAction} busy=${busy} />` : route === 'packs' ? html`<${PacksPage} packs=${data.packs} policies=${data.policies} onRefresh=${() => loadRoute('packs')} />` : route === 'settings' ? html`<${SettingsPage} username=${username} ipinfo=${data.ipinfo} frontendDetection=${data.frontendDetection} ipListAPI=${data.ipListAPI} onRotateEntry=${rotateEntry} onSaveIPInfo=${saveIPInfo} onSaveFrontendDetection=${saveFrontendDetection} onSaveIPListAPI=${saveIPListAPI} onRotateIPListAPI=${rotateIPListAPI} />` : html`<${DashboardControlPage} dashboard=${data.dashboard} instances=${data.instances} onNavigate=${onNavigate} onRefresh=${() => loadRoute('dashboard')} onOpenEvent=${openEvent} animationKey=${dashboardEntryKey} />`
