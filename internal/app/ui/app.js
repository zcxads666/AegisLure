...
function App() {
  ...
  const loadRoute = useCallback(async (target = route, quiet = false, overrideParams = null) => {
    ...
    try {
      ...
      else if (target === 'settings') { const [ipinfo, frontendDetection] = await Promise.all([request('ipinfo', options), request('frontend-detection', options)]); if (!current()) return null; setData((value) => ({ ...value, ipinfo, frontendDetection })) }
