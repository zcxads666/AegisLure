...
function PacksPage({ packs, policies, onRefresh }) {
  ...
  const togglePolicy = async (policy) => {
    ...
    try {
      ...
      setPolicyMessage(`${policy.provider} 登录页入口已${policy.enabled ? '关闭' : '开启'}。`)
      await onRefresh()
    } catch (error) {
